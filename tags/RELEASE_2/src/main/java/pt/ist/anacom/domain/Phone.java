...
import pt.ist.anacom.exception.InvalidBalanceOperationException;
import pt.ist.anacom.exception.PhoneIsBUSYException;
import pt.ist.anacom.exception.PhoneIsOFFException;
import pt.ist.anacom.exception.PhoneNumberIncorrectException;
...
public abstract class Phone extends Phone_Base {
	...
	public void increaseBalanceBy(int valueToIncrease) {
		...
		if ((balance += valueToIncrease) > 100 || valueToIncrease < 0)
			throw new InvalidBalanceOperationException(this);
		setBalance(balance);
	}
	...
	public void addSentSMS(SMS sms) {
		...
		if (this.getState() == State.MobileStateOFF)
			throw new PhoneIsOFFException(this);

		if (this.getState().equals(State.MobileStateBUSY))
			throw new PhoneIsBUSYException(this);

        ...
        rpcService.setPhoneState(dto, new AsyncCallback<Void>() {
            ...
            public void onFailure(Throwable caught) {
                ...
                if (caught instanceof NoSuchPhoneException) {
                    ...
                    OperatorPrefixDoesNotExistException ex = (OperatorPrefixDoesNotExistException) caught;
                    Window.alert(ex.getMessage());
                } else if (caught instanceof CouldNotSetStateWhileCommunicationActiveException) {
                	CouldNotSetStateWhileCommunicationActiveException ex = (CouldNotSetStateWhileCommunicationActiveException) caught;
                	Window.alert(ex.getMessage());
                }
                else {
                    Window.alert("ERROR:Unknown Exception: " + caught.getMessage());
        ...
        rpcService.getLastMadeCommunication(dto, new AsyncCallback<LastCommunicationDto>() {
            ...
            public void onSuccess(LastCommunicationDto result) {
                ...
                getLastCommunicationMadePanel.getLastCommunicationMadeTable().setText(1,
                                                                                      0,
                                                                                      "Last Communication was a " + result.getCommunicationType()
                                                                                              + " to " + result.getDestinationPhoneNumber()
                                                                                              + " that costed " + result.getCost()
                                                                                              + " and had length of " + result.getLength());
            }

...
public class ApplicationServerWebService implements AnacomPortType {
	...
	public static void init(final String operatorPrefix,
			final String replicaId, final String filePath) {
		System.out.println(".............STARTING Anacom " + operatorPrefix
				+ " SERVER........... " + replicaId);
		...
		try {
			...
			String publicPath = properties.getProperty("publicKeyPath")
					+ operatorPrefix + "-" + replicaId + "public.dat";
			String privatePath = properties.getProperty("privateKeyPath")
					+ operatorPrefix + "-" + replicaId + "private.dat";
			...
			OperatorCertificateInfoDto dto = new OperatorCertificateInfoDto(
					operator, SecurityData.encode64(pubKey.getEncoded()),
			...
			CertificateContents certificateContents = (CertificateContents) SecurityData
					.deserialize(SecurityData.decode64(certString));
			SignedCertificate actualCertificate = new SignedCertificate(
					certificateContents, certSignature);
			...
			bridge.revokeCertificate(new OperatorDto(operatorINFO.getEntityID()));

			dto = new OperatorCertificateInfoDto(operator,
					SecurityData.encode64(pubKey.getEncoded()),
					SecurityData.VALIDITY);
			certificateDto = bridge.signCertificate(dto);
			certString = certificateDto.getCertificate();
			certSignature = certificateDto.getSignature();
			certificateContents = (CertificateContents) SecurityData
					.deserialize(SecurityData.decode64(certString));
			actualCertificate = new SignedCertificate(certificateContents,
					certSignature);
			operatorINFO.setActualCertificate(actualCertificate);
	...
	public int getReplicaVersion() {
		...
		ReplicaVersionDto versionDto = versionService
				.getReplicaVersionServiceResult();
	...
	@Override
	public int registerOperator(OperatorDetailedType registerOperatorInput)
			throws InvalidTaxRemoteException, OperatorNullNameRemoteException,
		...
		try {
			...
			if (registerOperatorInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] CRIAR OPERATOR NO SERVER: "
								+ registerOperatorInput.getOperatorPrefix()
								+ " version DTO: "
								+ registerOperatorInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(registerOperatorInput
										.getVersion()));
				...
				if (!replicaIsUpToDate(registerOperatorInput.getVersion())) {
					...
					OperatorDetailedDto dto = new OperatorDetailedDto(
							registerOperatorInput.getOperatorPrefix(),
							registerOperatorInput.getOperatorName(),
							registerOperatorInput.getTax(),
							registerOperatorInput.getTaxVoice(),
							registerOperatorInput.getTaxSMS(),
							registerOperatorInput.getTaxVideo(),
							registerOperatorInput.getTaxBonus());
					// Run command
					RegisterNewOperatorService service = new RegisterNewOperatorService(
							dto);
					service.execute();

					// Update replica version
					ReplicaVersionDto replicaVersionDto = new ReplicaVersionDto(
							registerOperatorInput.getVersion());
					SetReplicaVersionService setReplicaVersionService = new SetReplicaVersionService(
							replicaVersionDto);
				...
				System.err
						.println("[ERR] You should not be here. Parameters equals null.");
			}
			...
			remoteExc.setOperatorPrefix(e.getOperatorPrefix());
			throw new OperatorPrefixWrongLengthRemoteException(e.getMessage(),
					remoteExc);
		} catch (OperatorPrefixAlreadyExistsException e) {
			OperatorSimpleType remoteExc = new OperatorSimpleType();
			remoteExc.setOperatorPrefix(e.getOperatorPrefix());
			throw new OperatorPrefixAlreadyExistsRemoteException(
					e.getMessage(), remoteExc);
			...
			remoteExc.setOperatorName(e.getOperatorName());
			throw new OperatorNameAlreadyExistsRemoteException(e.getMessage(),
					remoteExc);
		}
		return 0;
	}
	...
	@Override
	public int registerPhone(PhoneDetailedType registerPhoneInput)
			throws OperatorPrefixDoesNotExistRemoteException,
		...
		try {
			...
			if (registerPhoneInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] CRIAR CONTACTO NO SERVER: "
								+ registerPhoneInput.getPhoneNumber()
								+ " version DTO: "
								+ registerPhoneInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(registerPhoneInput
										.getVersion()));

				if (!replicaIsUpToDate(registerPhoneInput.getVersion())) {

					// Create DTOs
					PhoneDetailedDto dto = new PhoneDetailedDto(
							registerPhoneInput.getOperatorPrefix(),
							registerPhoneInput.getPhoneNumber(),
							AnacomData
									.convertIntToPhoneTypeEnum(registerPhoneInput
											.getPhoneGen()));
					// Run command
					RegisterNewPhoneService service = new RegisterNewPhoneService(
							dto);
					service.execute();
			...
			throw new PhoneAndOperatorPrefixDoNotMatchRemoteException(
					e.getMessage(), remoteExc);
		} catch (PhoneAlreadyExistsException e) {
			PhoneSimpleType remoteExc = new PhoneSimpleType();
			remoteExc.setPhoneNumber(e.getPhoneNumber());
			throw new PhoneAlreadyExistsRemoteException(e.getMessage(),
					remoteExc);
		} catch (OperatorPrefixDoesNotExistException e) {
			OperatorSimpleType remoteExc = new OperatorSimpleType();
			remoteExc.setOperatorPrefix(e.getOperatorPrefix());
			throw new OperatorPrefixDoesNotExistRemoteException(e.getMessage(),
					remoteExc);
		} catch (PhoneNumberWrongLengthException e) {
			PhoneSimpleType remoteExc = new PhoneSimpleType();
			remoteExc.setPhoneNumber(e.getPhoneNumber());
			throw new PhoneNumberWrongLengthRemoteException(e.getMessage(),
					remoteExc);
		}
		return 0;
	}
	...
	@Override
	public int setPhoneState(PhoneAndStateType setPhoneStateInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		try {
			if (setPhoneStateInput != null) {

				Logger.getLogger(this.getClass()).info(
						"[ANACOM] MUDAR ESTADO TELEMOVEL NO SERVER: "
								+ setPhoneStateInput.getPhoneNumber()
								+ " version DTO: "
								+ setPhoneStateInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(setPhoneStateInput
										.getVersion()));

				if (!replicaIsUpToDate(setPhoneStateInput.getVersion())) {

					// Create DTOs
					PhoneAndStateDto dto = new PhoneAndStateDto(
							setPhoneStateInput.getPhoneNumber(),
							AnacomData.convertIntToStateEnum(setPhoneStateInput
									.getPhoneState()));
					// Run command
	...
	@Override
	public int cancelRegisteredPhone(PhoneSimpleType cancelRegisteredPhoneInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		...
		try {
			...
			if (cancelRegisteredPhoneInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] CANCELAR REGISTO TELEMOVEL NO SERVER: "
								+ cancelRegisteredPhoneInput.getPhoneNumber()
								+ " version DTO: "
								+ cancelRegisteredPhoneInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(cancelRegisteredPhoneInput
										.getVersion()));

				if (!replicaIsUpToDate(cancelRegisteredPhoneInput.getVersion())) {

					PhoneSimpleDto dto = new PhoneSimpleDto(
							cancelRegisteredPhoneInput.getPhoneNumber());
					CancelRegisteredPhoneService service = new CancelRegisteredPhoneService(
							dto);
					service.execute();
	...
	@Override
	public StateType getPhoneState(PhoneSimpleType getPhoneStateInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		try {
			if (getPhoneStateInput != null) {

				System.out.println("[jUDDI]  GET PHONE STATE NO SERVER: "
						+ getPhoneStateInput.getPhoneNumber());
	...
	@Override
	public BalanceAndPhoneListType getBalanceAndPhoneList(
			OperatorSimpleType getBalanceAndPhoneListInput)
			throws OperatorPrefixDoesNotExistRemoteException {

		try {
			if (getBalanceAndPhoneListInput != null) {

				System.out.println("[jUDDI]  GET BALANCE NO SERVER: "
						+ getBalanceAndPhoneListInput.getOperatorPrefix());

				OperatorSimpleDto operatorDto = new OperatorSimpleDto(
						getBalanceAndPhoneListInput.getOperatorPrefix());

				// Execute service
				GetBalanceAndPhoneListService service = new GetBalanceAndPhoneListService(
						operatorDto);
				service.execute();
				BalanceAndPhoneListType phoneAndBalanceListType = new BalanceAndPhoneListType();

				// fill in dto arrayList
				for (BalanceAndPhoneDto balanceAndPhoneDto : service
						.getBalanceAndPhoneListServiceResult().getPhoneList()) {

					BalanceAndPhoneType balanceAndPhoneType = new BalanceAndPhoneType();

					balanceAndPhoneType.setPhoneNumber(balanceAndPhoneDto
							.getPhoneNumber());
					balanceAndPhoneType.setBalance(balanceAndPhoneDto
							.getBalance());

					phoneAndBalanceListType.getPhoneList().add(
							balanceAndPhoneType);
				}
	...
	@Override
	public SMSPhoneReceivedListType getSMSPhoneReceivedList(
			PhoneSimpleType getSMSPhoneReceivedListInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		...
		try {
			if (getSMSPhoneReceivedListInput != null) {
				...
				System.out.println("[jUDDI]  GET SMS RECEIVES LIST NO SERVER: "
						+ getSMSPhoneReceivedListInput.getPhoneNumber());

				PhoneSimpleDto phoneDto = new PhoneSimpleDto(
						getSMSPhoneReceivedListInput.getPhoneNumber());

				// Execute service
				GetSMSPhoneReceivedListService service = new GetSMSPhoneReceivedListService(
						phoneDto);
				service.execute();
				SMSPhoneReceivedListType smsPhoneReceivedListType = new SMSPhoneReceivedListType();

				// fill in dto arrayList
				System.out.println("IMPRIMIR LISTAAAAA"
						+ service.getSMSPhoneReceivedListServiceResult());
				for (SMSDto smsDto : service
						.getSMSPhoneReceivedListServiceResult().getSmsList()) {

					SMSType smsType = new SMSType();

					smsType.setSourcePhoneNumber(smsDto.getSourcePhoneNumber());
					smsType.setDestinationPhoneNumber(smsDto
							.getDestinationPhoneNumber());
					smsType.setMessage(smsDto.getMessage());

					smsPhoneReceivedListType.getSmsList().add(smsType);
				}
				System.out.println("ESTARA VAZIA?????"
						+ smsPhoneReceivedListType.getSmsList());
	...
	public int sendSMS(SMSType sendSMSInput) throws NoSuchPhoneRemoteException,
			...
			OperatorPrefixDoesNotExistRemoteException,
			PhoneStateRemoteException, SMSInvalidTextRemoteException,
		...
		try {
			...
			if (sendSMSInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] ENVIAR SMS PARA TELEMOVEL DESTINO: "
								+ sendSMSInput.getSourcePhoneNumber() + "to"
								+ sendSMSInput.getDestinationPhoneNumber()
								+ " version DTO: " + sendSMSInput.getVersion()
								+ " version SERVER:" + getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(sendSMSInput.getVersion()));

				if (!replicaIsUpToDate(sendSMSInput.getVersion())) {

					SMSDto smsDto = new SMSDto(
							sendSMSInput.getSourcePhoneNumber(),
							sendSMSInput.getDestinationPhoneNumber(),
							sendSMSInput.getMessage());

					System.out.println("ESTOU NO SEND SMS" + smsDto);

					ProcessSendSMSService sendService = new ProcessSendSMSService(
							smsDto);
					sendService.execute();

					// Update replica version
					ReplicaVersionDto replicaVersionDto = new ReplicaVersionDto(
							sendSMSInput.getVersion());
					SetReplicaVersionService setReplicaVersionService = new SetReplicaVersionService(
							replicaVersionDto);
					setReplicaVersionService.execute();
			...
			throw new NegativeBalanceValueRemoteException(e.getMessage(),
					remoteExc);
		} catch (InsuficientBalanceException e) {
			BalanceAndPhoneType remoteExc = new BalanceAndPhoneType();
			remoteExc.setPhoneNumber(e.getPhoneNumber());
			remoteExc.setBalance(e.getPhoneBalance());
			throw new InsuficientBalanceRemoteException(e.getMessage(),
					remoteExc);
	...
	@Override
	public int receiveSMS(SMSType sendSMSInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException,
			PhoneStateRemoteException, SMSInvalidTextRemoteException {
		try {
			...
			if (sendSMSInput != null) {
				...
				if (!replicaIsUpToDate(sendSMSInput.getVersion())) {
					...
					System.out.println("ESTOU NO RECEIVE SMS" + smsDto);

					ProcessReceiveSMSService receiveService = new ProcessReceiveSMSService(
							smsDto);
	...
	@Override
	public BalanceType getPhoneBalance(PhoneSimpleType getPhoneBalanceInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		try {
			...
			if (getPhoneBalanceInput != null) {
				...
				System.out.println("[jUDDI]  GET BALANCE NO SERVER: "
						+ getPhoneBalanceInput.getPhoneNumber());
	...
	@Override
	public int increasePhoneBalance(
			BalanceAndPhoneType increasePhoneBalanceInput)
			throws BalanceLimitExceededRemoteException,
			NoSuchPhoneRemoteException, NegativeBalanceValueRemoteException,
			OperatorPrefixDoesNotExistRemoteException {
		...
		try {
			if (increasePhoneBalanceInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] AUMENTAR SALDO NO SERVER: "
								+ increasePhoneBalanceInput.getPhoneNumber()
								+ " version DTO: "
								+ increasePhoneBalanceInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(increasePhoneBalanceInput
										.getVersion()));

				if (!replicaIsUpToDate(increasePhoneBalanceInput.getVersion())) {

					// Create DTOs
					BalanceAndPhoneDto dto = new BalanceAndPhoneDto(
							increasePhoneBalanceInput.getPhoneNumber(),
							increasePhoneBalanceInput.getBalance());

					// Run command
					IncreaseBalanceService service = new IncreaseBalanceService(
							dto);
					service.execute();
			...
			remoteExc.setPhoneNumber(e.getPhoneNumber());
			remoteExc.setBalance(e.getPhoneBalance());
			throw new BalanceLimitExceededRemoteException(e.getMessage(),
					remoteExc);
		}
		return 0;
	}
	...
	@Override
	public LastCommunicationType getPhoneLastMadeCommunication(
			PhoneSimpleType getPhoneLastMadeCommunicationInput)
			throws NoSuchPhoneRemoteException,
		...
		try {
			...
			if (getPhoneLastMadeCommunicationInput != null) {
				...
				System.out.println("[jUDDI]  GET BALANCE NO SERVER: "
						+ getPhoneLastMadeCommunicationInput.getPhoneNumber());

				PhoneSimpleDto phoneDto = new PhoneSimpleDto(
						getPhoneLastMadeCommunicationInput.getPhoneNumber());
				GetLastMadeCommunicationService service = new GetLastMadeCommunicationService(
						phoneDto);
				service.execute();
				LastCommunicationDto communicationDto = service
						.getLastMadeCommunicationServiceResult();

				LastCommunicationType communicationType = new LastCommunicationType();
				communicationType.setType(communicationDto
						.getCommunicationType().ordinal());
				communicationType.setDestinationPhoneNumber(communicationDto
						.getDestinationPhoneNumber());
			...
			PhoneSimpleType remoteExc = new PhoneSimpleType();
			remoteExc.setPhoneNumber(e.getPhoneNumber());
			throw new NoCommunicationsMadeYetRemoteException(e.getMessage(),
					remoteExc);
		}
	}

	@Override
	public int startSendVoiceCall(CommunicationType startSendVoiceCallInput)
			throws NoSuchPhoneRemoteException,
			NegativeBalanceValueRemoteException,
			OperatorPrefixDoesNotExistRemoteException,
			PhoneStateRemoteException, InsuficientBalanceRemoteException {
		try {
			if (startSendVoiceCallInput != null) {

				Logger.getLogger(this.getClass()).info(
						"[ANACOM] COMECAR CHAMADA NO SERVER: "
								+ startSendVoiceCallInput
										.getSourcePhoneNumber()
								+ " version DTO: "
								+ startSendVoiceCallInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(startSendVoiceCallInput
										.getVersion()));

				if (!replicaIsUpToDate(startSendVoiceCallInput.getVersion())) {

					// Create DTOs
					CommunicationDto voiceDto = new CommunicationDto(
							startSendVoiceCallInput.getSourcePhoneNumber(),
							startSendVoiceCallInput.getDestinationPhoneNumber());

					// Run command
					ProcessStartSourceVoiceCallService sourceService = new ProcessStartSourceVoiceCallService(
							voiceDto);
	...
	@Override
	public int startReceiveVoiceCall(
			CommunicationType startReceiveVoiceCallInput)
			throws NoSuchPhoneRemoteException,
			OperatorPrefixDoesNotExistRemoteException,
		...
		try {
			if (startReceiveVoiceCallInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] COMECAR RECEBER CHAMADA NO SERVER: "
								+ startReceiveVoiceCallInput
										.getSourcePhoneNumber()
								+ " version DTO: "
								+ startReceiveVoiceCallInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(startReceiveVoiceCallInput
										.getVersion()));
				...
				if (!replicaIsUpToDate(startReceiveVoiceCallInput.getVersion())) {
					...
					// Create DTOs
					CommunicationDto voiceDto = new CommunicationDto(
							startReceiveVoiceCallInput.getSourcePhoneNumber(),
							startReceiveVoiceCallInput
									.getDestinationPhoneNumber());

					// Run command
					ProcessStartDestinationVoiceCallService destinationService = new ProcessStartDestinationVoiceCallService(
							voiceDto);
	...
	@Override
	public int endSendVoiceCall(CommunicationDurationType endSendVoiceCallInput)
			throws NoSuchPhoneRemoteException,
			NegativeBalanceValueRemoteException,
			NoActiveCommunicationRemoteException,
			OperatorPrefixDoesNotExistRemoteException,
			InvalidDurationRemoteException, InsuficientBalanceRemoteException {

		try {
			if (endSendVoiceCallInput != null) {

				Logger.getLogger(this.getClass()).info(
						"[ANACOM] ACABAR CHAMADA NO SERVER: "
								+ endSendVoiceCallInput.getSourcePhoneNumber()
								+ " version DTO: "
								+ endSendVoiceCallInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(endSendVoiceCallInput
										.getVersion()));

				if (!replicaIsUpToDate(endSendVoiceCallInput.getVersion())) {

					// Create DTOs
					CommunicationDurationDto voiceDurationDto = new CommunicationDurationDto(
							endSendVoiceCallInput.getSourcePhoneNumber(),
							endSendVoiceCallInput.getDestinationPhoneNumber(),
							endSendVoiceCallInput.getDuration());

					// Run command
					ProcessEndSourceVoiceCallService sourceService = new ProcessEndSourceVoiceCallService(
							voiceDurationDto);
					sourceService.execute();
			...
			remoteExc.setDestinationPhoneNumber(e.getDestinationPhoneNumber());
			remoteExc.setSourcePhoneNumber(e.getSourcePhoneNumber());
			throw new NoActiveCommunicationRemoteException(e.getMessage(),
					remoteExc);
		}
	...
	@Override
	public int endReceiveVoiceCall(
			CommunicationDurationType endReceiveVoiceCallInput)
			throws NoSuchPhoneRemoteException,
			NoActiveCommunicationRemoteException,
			OperatorPrefixDoesNotExistRemoteException,
		...
		try {
			if (endReceiveVoiceCallInput != null) {
				...
				Logger.getLogger(this.getClass()).info(
						"[ANACOM] ACABAR RECEBER CHAMADA NO SERVER: "
								+ endReceiveVoiceCallInput
										.getSourcePhoneNumber()
								+ " version DTO: "
								+ endReceiveVoiceCallInput.getVersion()
								+ " version SERVER:"
								+ getReplicaVersion()
								+ " "
								+ replicaIsUpToDate(endReceiveVoiceCallInput
										.getVersion()));
				...
				if (!replicaIsUpToDate(endReceiveVoiceCallInput.getVersion())) {
					...
					// Create DTOs
					CommunicationDurationDto voiceDurationDto = new CommunicationDurationDto(
							endReceiveVoiceCallInput.getSourcePhoneNumber(),
							endReceiveVoiceCallInput
									.getDestinationPhoneNumber(),
							endReceiveVoiceCallInput.getDuration());
					// Run command
					ProcessEndDestinationVoiceCallService destinationService = new ProcessEndDestinationVoiceCallService(
							voiceDurationDto);
					destinationService.execute();

	...
	public boolean handleMessage(SOAPMessageContext messageContext) {
		...
		if (!outboundProperty.booleanValue()) {
			...
			try {
				...
				if (soapHeader != null) {
					...
					SOAPElement signature = SecurityData.getSOAPHeaderElement(
							soapHeader, "signature");
					if (signature != null) {
						...
						PublicKey publicKey = SecurityData
								.getPublicKey(entityINFO.getCaPublicKey());
		...
		// Outbound message
			...
			try {
				...
				Properties properties = SecurityData
						.readPropertiesFile(this.entityINFO.getPath());
				String privatePath = properties.getProperty("privateKeyPath")
						+ this.entityINFO.getEntityID() + "private.dat";

				ByteArrayOutputStream byteOut = new ByteArrayOutputStream();
				message.writeTo(byteOut);

				byte[] cipherDigest = SecurityData.makeDigitalSignature(
						byteOut.toByteArray(),
...
//					NodeList nodeList = bodyNodes.item(i).getChildNodes();
//
//					for (int j = 0; j < nodeList.getLength(); j++) {
//						Node content = nodeList.item(j);
//
//						System.out.println("NAME = " + content.getNodeName()

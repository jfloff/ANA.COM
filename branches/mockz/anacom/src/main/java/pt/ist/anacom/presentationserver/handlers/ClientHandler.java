...
public class ClientHandler implements SOAPHandler<SOAPMessageContext> {
    ...
    public boolean handleMessage(SOAPMessageContext context) {
        ...
        if (!outboundProperty.booleanValue()) {
            ...
            try {
                ...
                if (soapHeader != null) {
                    ...
                    if (signature != null) {
                        ...
                        CertificateListDto listDto = PresentationServer.certificateAuthority.getOperatorRevokedCertificateList(dto);

                        for (CertificateDto certDto : listDto.getCertificateList()) {
                            CertificateContents certContents = (CertificateContents) SecurityData.deserialize(SecurityData.decode64(certDto.getCertificate()));
                            SignedCertificate signed = new SignedCertificate(certContents, certDto.getSignature());
                            PresentationServer.clientINFO.getRevokedList().add(signed);
                        }

                        if (!PresentationServer.clientINFO.getRevokedList().contains(certificate))
            ...
            try {
                SOAPMessageContext messageContext = context;

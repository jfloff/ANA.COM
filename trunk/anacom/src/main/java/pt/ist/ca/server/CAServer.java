...
public class CAServer {
    ...
    private void ourMain() {
        ...
        try {
            ...
            Logger.getLogger(this.getClass()).info("CERTIFICATE " + dto.toString());

            revokeCertificateCommand("Server1");
            Logger.getLogger(this.getClass()).info(getRevokedCertificateList().getCertificateList());
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
        }
    }

    private CertificateListDto getRevokedCertificateList() {
        try {
            CertificateListDto listDto = null;
            listDto = serviceBridge.getRevokedCertificateList();
            return listDto;
